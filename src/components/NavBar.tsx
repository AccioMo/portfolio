...
export default function NavBar({logo}: {logo: boolean | false}) {
    ...
    return (
        <>
        {/* Desktop Navigation */}
        <div className="hidden md:flex absolute font-sans w-full justify-center m-auto px-8 lg:px-16 py-8 lg:py-12 z-40 text-sm lg:text-base text-primary" data-project-area="true">
            <div className="flex justify-between min-w-56 gap-4 lg:gap-6">
                ...
                <Link href="/works" className={`flex items-center group transition-all duration-300 ${
                  pathname === '/works' ? 'text-primary' : 'text-secondary group-hover:text-primary'
                }`}>
                    <span>stuff</span>
                </Link>

                <Link href="/about" className={`flex items-center group transition-all duration-300 ${
                  pathname === '/about' ? 'text-primary' : 'text-secondary group-hover:text-primary'
                }`}>
                    <span>me</span>
                </Link>

                <Link href="/contact" className={`flex items-center group transition-all duration-300 ${
        ...
        <div className="md:hidden absolute top-6 right-0 z-40 font-sans" data-project-area="true">
            <div className={`flex items-center gap-4 pr-6 transition-transform duration-400 ease-in-out ${
                isMenuOpen ? 'translate-x-0' : 'translate-x-[calc(100%-2.5rem)]'
            }`}>
                {/* Triangle Toggle Button */}
                <button
                    onClick={() => setIsMenuOpen(!isMenuOpen)}
                    className={`text-primary hover:text-secondary transition-all flex-shrink-0 ${
                        isMenuOpen ? 'scale-110 opacity-100' : 'scale-100 opacity-70'
                    }`}
